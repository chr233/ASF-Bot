...
public static class LoggerExtensions
{
    ...
    public static void LogMessage(this ILogger logger, Message message)
    {
        var content = message.Type switch {
            ...
            MessageType.Photo => $"[图片] {message.Photo} {message.Caption}",
            ...
            MessageType.Sticker => $"[贴纸] {message.Sticker!.SetName}.{message.Sticker!.Emoji}",
            ...
            MessageType.SupergroupChatCreated => "[群升级为超级群]",
            MessageType.GroupChatCreated => "[超级群降级为群]",
        ...
        var userDetail = $"{message.From?.FullName()}({message.From?.UserID()})";
        var groupDetail = message.MessageThreadId != null ? $"{chat.FullChatProfile()} {message.MessageThreadId}" : chat.FullChatProfile();

        var chatFrom = chat.Type switch {
            ChatType.Private => $"私聊 | {userDetail}",
            ChatType.Group => $"群组 | {groupDetail} | {userDetail}",
            ChatType.Channel => $"频道 | {groupDetail} | {userDetail}",
            ChatType.Supergroup => $"群组 | {groupDetail} | {userDetail}",
            ChatType.Sender => $"Query | {groupDetail} | {userDetail}",
            _ => $"未知 | {groupDetail} | {userDetail}",
        };

        logger.LogInformation("{chatFrom}\r\n{content}", chatFrom, content);
    }
    ...
    {
        var user = callbackQuery.From.UserToString();
    ...
    {
        var user = inlineQuery.From.UserToString();
